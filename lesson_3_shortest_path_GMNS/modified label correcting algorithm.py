...
g_adjacent_arc_list={} #set of arcs emanating from some node
g_shortest_path=[] #set of shortest paths
g_node_status=[] #set of node status,1: in LIST,0：not in LIST
g_origin=None  #set of source node
g_number_of_nodes=0
...
"""import network data and initialize corresponding variables"""
#read data of nodes
...
while len(SE_LIST):
    head=random.sample(SE_LIST,1)[0]#randomly choose arc from SE_LIST
    SE_LIST.remove(head)#remove the chosen node
