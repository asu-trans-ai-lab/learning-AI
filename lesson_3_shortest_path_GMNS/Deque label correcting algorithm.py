...
"""finding shortest path: scan arcs and check their distance labels"""
while len(SE_LIST):
    head=SE_LIST[0] # pick first node from scan eligible list
    SE_LIST.pop(0) # remove that node
    g_node_status[head-1]=2
    ...
    for tail in adjacent_arc_list:
        if node_label_cost[tail-1]>node_label_cost[head-1]+Distance[head-1,tail-1]:
            ...
            elif g_node_status[tail-1]==2:
                SE_LIST.insert(0,tail)
                g_node_status[tail-1]=1
